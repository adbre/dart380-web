...
Ra180MenuBase.prototype.update = function () {
    ...
    this._smallDisplay.set(sprintf('%s%s%s', prefix, canEdit ? ':' : '=', value));
    this._smallDisplay.setBlinking(canEdit ? prefix.length : -1);
    this._smallDisplay.setCursor(this._isEditing ? prefix.length + 1 + this._cursor : -1);
};
